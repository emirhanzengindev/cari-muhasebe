...
export default function EditAccount() {
  ...
  const { accounts, fetchAccounts, updateAccount } = useCurrentAccountsStore();
  ...
  useEffect(() => {
    ...
    loadAccount();
  }, [accountId]); // fetchAccounts ve accounts bağımlılıklarını kaldır
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/current-accounts/${accountId}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        <Link href={`/current-accounts/${accountId}`} className="text-blue-600 hover:text-blue-800 flex items-center">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clipRule="evenodd" />
          </svg>
          Hesap Detayı
        </Link>
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Hesabı Düzenle</h1>
      </div>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg">
        <form onSubmit={handleSubmit}>
          ...
          <div className="px-4 py-3 bg-gray-50 text-right sm:px-6">
            <button
              type="button"
              onClick={() => router.push(`/current-accounts/${accountId}`)}
